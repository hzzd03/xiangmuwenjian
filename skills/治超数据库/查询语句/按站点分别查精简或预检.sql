...
-- 查询一：只看首检（精简）数据
-- =============================================
DECLARE @stationNO1 VARCHAR(50) = '13082301'  -- ← 站点编号
...
INNER JOIN (
    ...
    WHERE fCheckStation = @stationNO1
) t ON bs.fNO = t.检测站编号
WHERE bs.fType = 0 AND bs.fNoUsed = 0
ORDER BY t.检测时间 DESC
GO

-- =============================================
-- 查询二：只看预检数据
-- =============================================
DECLARE @stationNO2 VARCHAR(50) = '13082301'  -- ← 站点编号
...
INNER JOIN (
    ...
    WHERE fCheckStation = @stationNO2
) t ON bs.fNO = t.检测站编号
WHERE bs.fType = 0 AND bs.fNoUsed = 0
ORDER BY t.检测时间 DESC
